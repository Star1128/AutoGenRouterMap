/*
  @license
	Rollup.js v3.10.0
	Thu, 13 Jun 2024 07:59:15 GMT - commit 2ac09b2ee33edf6cbfcc7e758c62207d8cf56053
...
function getOriginalLocation(sourcemapChain, location, originalSourcemap) {
    const filteredSourcemapChain = sourcemapChain.length === 0
        ? [originalSourcemap]
        : sourcemapChain.filter((sourcemap) => !!sourcemap.mappings);
    traceSourcemap: while (filteredSourcemapChain.length > 0) {
        const sourcemap = filteredSourcemapChain.pop();
        const line = sourcemap.mappings ? sourcemap.mappings[location.line - 1] : undefined;
        if (line) {
...
function timeStartImpl(label, level = 3) {
    ...
    const startTime = Number(process$1.hrtime.bigint());
...
function timeEndImpl(label, level = 3) {
    ...
    if (timer !== undefined) {
        ...
        timer.endTime = Number(process$1.hrtime.bigint());
...
function getTimings() {
    const newTimings = {};
    for (const [label, { frequency, startTime, time, endTime, memory, startMemory, totalMemory }] of timers) {
        newTimings[label] = [time, startTime, endTime, frequency, memory, startMemory, totalMemory];
    }
    return newTimings;
}
let timeStart = NOOP;
    ...
    for (const hook of TIMED_PLUGIN_HOOKS) {
        if (hook in plugin) {
            ...
            const handler = function (...parameters) {
                ...
                timeEnd(timerLabel, 4, JSON.stringify({ params: extractFileInfo(hook, parameters) }));
                return result;
            };
            let hookFunction;
...
function extractFileInfo(hook, parameters) {
    switch (hook) {
        case 'resolveId':
        case 'load':
...
class Module {
    constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta, assertions) {
        ...
        this.type2nodeInfo = new Map();
        this.allExportNames = null;
        this.ast = null;
        this.cacheAst = {
            dynamicImportCache: [],
            exportAllDeclarationCache: [],
            exportNamedDeclarationCache: [],
            importCache: []
        };
        ...
        this.exportAllDeclarationHandler = (node) => {
            this.addExportAllDeclarationWithGenerateModuleGraphOnly(node);
            this.updateType2nodeInfo('ExportAllDeclaration', node);
            this.cacheAst.exportAllDeclarationCache.push(node);
        };
        this.exportNamedDeclarationHandler = (node) => {
            this.addExportNamedDeclarationWithGenerateModuleGraphOnly(node);
            this.updateType2nodeInfo('ExportNamedDeclaration', node);
            this.cacheAst.exportNamedDeclarationCache.push(node);
        };
        this.importDeclarationHandler = (node) => {
            this.addImportWithGenerateModuleGraphOnly(node);
            this.updateType2nodeInfo('ImportDeclaration', node);
            this.cacheAst.importCache.push(node);
        };
        this.importExpressionHandler = (node) => {
            this.addDynamicImportWithGenerateModuleGraphOnly(node);
            this.updateType2nodeInfo('ImportExpression', node);
            this.cacheAst.dynamicImportCache.push(node);
        };
        this.updateType2nodeInfo = (nodeType, node) => {
            if (this.type2nodeInfo.has(nodeType)) {
                this.type2nodeInfo.get(nodeType).add(node);
            }
            else {
                this.type2nodeInfo.set(nodeType, new Set([node]));
            }
        };
        ...
        this.info = {
            ...
            getNodeByType(...nodeTypes) {
                const nodes = new Map();
                for (const nodeType of nodeTypes) {
                    if (module.type2nodeInfo.has(nodeType) && !nodes.has(nodeType)) {
                        nodes.set(nodeType, module.type2nodeInfo.get(nodeType));
                    }
                }
                return nodes;
            },
            ...
            get importedIdMaps() {
                const map = {};
                for (const [key, resolvedId] of Object.entries(module.resolvedIds)) {
                    map[key] = resolvedId.id;
                }
                return map;
            },
    ...
    linkImports() {
        if (this.options.generateModuleGraphOnly) {
            return;
        }
    ...
    render(options) {
        if (!this.magicString) {
            return error({
                message: 'In generateModuleGraphOnly mode, rendering can never happen.'
            });
        }
    ...
    setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, cacheAst, ...moduleOptions }, useCacheAst = false) {
        ...
        let moduleAst = null;
        if (!(this.options.generateModuleGraphOnly && useCacheAst)) {
            moduleAst = ast ?? this.tryParse();
        }
        ...
        if (this.options.generateModuleGraphOnly) {
            this.processModuleGraphAst(moduleAst, cacheAst, useCacheAst);
        }
        else {
    ...
    toJSON() {
        ...
        this.options.generateModuleGraphOnly
            ? (module.cacheAst = this.cacheAst)
            : (module.ast = this.info.ast);
        return module;
    }
    ...
    addDynamicImportWithGenerateModuleGraphOnly(node) {
        let argument = node.source;
    ...
    addExportAllDeclarationWithGenerateModuleGraphOnly(node) {
        const source = node.source.value;
        this.addSource(source, node);
    }
    addExportNamedDeclarationWithGenerateModuleGraphOnly(node) {
    ...
    addImportWithGenerateModuleGraphOnly(node) {
        const source = node.source.value;
        this.addSource(source, node);
        for (const specifier of node.specifiers) {
            const name = specifier.type === 'ImportDefaultSpecifier'
                ? 'default'
    ...
    addLocationToLogProps(properties, pos) {
        ...
        if (location) {
            ...
            try {
                ({ column, line } = getOriginalLocation(this.sourcemapChain, { column, line }, this.originalSourcemap));
    ...
    processModuleGraphAst(moduleAst, cacheFileAst, useCacheAst) {
        if (useCacheAst) {
            for (const node of cacheFileAst.exportAllDeclarationCache) {
                this.exportAllDeclarationHandler(node);
            }
            for (const node of cacheFileAst.exportNamedDeclarationCache) {
                this.exportNamedDeclarationHandler(node);
            }
            for (const node of cacheFileAst.importCache) {
                this.importDeclarationHandler(node);
            }
            for (const node of cacheFileAst.dynamicImportCache) {
                this.importExpressionHandler(node);
            }
        }
        else {
            simple(moduleAst, {
                ExportAllDeclaration: node => this.exportAllDeclarationHandler(node),
                ExportNamedDeclaration: node => this.exportNamedDeclarationHandler(node),
                ImportDeclaration: node => this.importDeclarationHandler(node),
                ImportExpression: node => this.importExpressionHandler(node)
            });
        }
    }
...
class Chunk {
    constructor(orderedModules, inputOptions, outputOptions, unsetOptions, pluginDriver, modulesById, chunkByModule, externalChunkByModule, facadeChunkByModule, includedNamespaces, manualChunkAlias, getPlaceholder, bundle, inputBase, snippets) {
        ...
        this.baseModule = orderedModules.length > 0 ? orderedModules[orderedModules.length - 1] : null;
    ...
    bind() {
        if (this.baseModule) {
            this.facadeModule = this.baseModule;
            this.facadeChunkByModule.set(this.baseModule, this);
            const { sanitizeFileName, preserveModules } = this.outputOptions;
            this.name =
                this.manualChunkAlias ??
                    sanitizeFileName(preserveModules
                        ? this.getPreserveModulesChunkNameFromModule(this.baseModule)
                        : getChunkNameFromModule(this.baseModule));
        }
    }
    ...
    getRenderedChunkInfo() {
        ...
        return (this.renderedChunkInfo = {
            ...
            chunkHash: (() => {
                const chunkOriginalCode = this.orderedModules.map(module => module.originalCode).join(' ');
                const hash = createHash();
                return hash.update(chunkOriginalCode).digest('hex');
            })(),
            ...
            exportsByName: (() => {
                ...
                for (const exportName of this.getExportNames()) {
                    ...
                    if (variable) {
                        exportsByName.set(exportName, { name: variable.name, renderName: variable.renderName });
                    }
                }
            ...
            importsPathMap: (() => {
                const importsPathMap = new Map();
                for (const [dependency, declaration] of this.getRenderedDependencies()) {
                    importsPathMap.set(declaration.importPath, resolveFileName(dependency));
                }
                return importsPathMap;
            })(),
    ...
    renderModules(fileName) {
        const { accessedGlobalsByScope, dependencies, exportNamesByVariable, facadeModule, includedNamespaces, inputOptions: { onwarn }, isEmpty, orderedModules, outputOptions, pluginDriver, renderedModules, snippets } = this;
        const { compact, dynamicImportFunction, format, freeze, namespaceToStringTag, preserveModules, bundleMode } = outputOptions;
        const { _, cnst, n } = snippets;
        ...
        for (const module of orderedModules) {
            ...
            if (module.isIncluded() || includedNamespaces.has(module)) {
                renderOptions.exportNamesByVariable =
                    bundleMode &&
                        module.info.isEntry &&
                        facadeModule !== null &&
                        module.id !== facadeModule.id
    ...
    setUpChunkImportsAndExportsForModule(module) {
        ...
        for (let variable of moduleImports) {
            ...
            const variableModule = variable.module;
            const chunk = this.chunkByModule.get(variableModule);
            if (chunk !== this &&
                (this.outputOptions.bundleMode ? !this.orderedModules.includes(variableModule) : true)) {
                this.imports.add(variable);
...
function getChunkAssignments(entries, manualChunkAliasByEntry, minChunkSize, bundleMode) {
    ...
    chunkDefinitions.push(...createChunks(allEntries, assignedEntriesByModule, minChunkSize, bundleMode));
...
function createChunks(allEntries, assignedEntriesByModule, minChunkSize, bundleMode) {
    const chunkModulesBySignature = bundleMode
        ? getBundleModeChunks(assignedEntriesByModule, allEntries)
        : getChunkModulesBySignature(assignedEntriesByModule, allEntries);
...
function getBundleModeChunks(assignedEntryPointsByModule, allEntries) {
    const chunkModules = Object.create(null);
    for (const entry of allEntries) {
        chunkModules[entry.id] = [entry];
    }
    for (const [module, assignedEntryPoints] of assignedEntryPointsByModule) {
        for (const entry of assignedEntryPoints) {
            if (module.info.isEntry && module.id === entry.id) {
                continue;
            }
            chunkModules[entry.id]?.push(module);
        }
    }
    return chunkModules;
}
...
class Bundle {
    ...
    async generateChunks(bundle, getHashPlaceholder) {
        const { bundleMode, experimentalMinChunkSize, inlineDynamicImports, manualChunks, preserveModules } = this.outputOptions;
        ...
        for (const { alias, modules } of inlineDynamicImports
            ...
            : preserveModules
                ...
                : getChunkAssignments(this.graph.entryModules, manualChunkAliasByEntry, experimentalMinChunkSize, bundleMode)) {
            sortByExecutionOrder(modules);
            const chunk = new Chunk(modules, this.inputOptions, this.outputOptions, this.unsetOptions, this.pluginDriver, this.graph.modulesById, bundleMode && manualChunkAliasByEntry.size === 0 ? new Map() : chunkByModule, externalChunkByModule, this.facadeChunkByModule, this.includedNamespaces, alias, getHashPlaceholder, bundle, inputBase, snippets);
            chunks.push(chunk);
        }
        for (const chunk of chunks) {
            chunk.link();
...
const lastReUsedFiles = new Set();
function getLastReUsedFiles() {
    return lastReUsedFiles;
}
function clearLastReUsedFiles() {
    lastReUsedFiles.clear();
}
class ModuleLoader {
    ...
    async addModuleSource(id, importer, module, isPreload) {
        let source;
        try {
            source = await this.graph.fileOperationQueue.run(async () => (await this.pluginDriver.hookFirst('load', [id])) ?? (await readFile(id, 'utf8')), !!isPreload);
        }
        catch (error_) {
        ...
        module.info.originalCode = sourceDescription.code;
        ...
        if (cachedModule &&
            ...
            lastReUsedFiles.add(module.id);
            module.setSource(cachedModule, true);
        }
        else {
            module.updateOptions(sourceDescription);
            module.info.isIncremental = true;
    ...
    async fetchModule({ assertions, id, meta, moduleSideEffects, syntheticNamedExports }, importer, isEntry, isPreload) {
        ...
        const loadPromise = this.addModuleSource(id, importer, module, isPreload).then(() => [
    ...
    getResolveStaticDependencyPromises(module) {
        ...
        return Array.from(module.sourcesWithAssertions, async ([source, assertions]) => [
            ...
            (module.resolvedIds[source] =
                module.resolvedIds[source] &&
                    (module.resolvedIds[source].external || fs.existsSync(module.resolvedIds[source].id))
                    ? module.resolvedIds[source]
...
const dataShare = {};
let asyncDoneEmitter = undefined;
let remainingAsyncOps = 0;
const asyncDone = 'done';
let donePromise = undefined;
function resolveDonePromise() {
    if (asyncDoneEmitter) {
        remainingAsyncOps = 0;
        asyncDoneEmitter.emit(asyncDone);
    }
}
function resetDonePromise() {
    asyncDoneEmitter = new EventEmitter();
    remainingAsyncOps = 0;
    donePromise = new Promise(resolve => {
        asyncDoneEmitter.on(asyncDone, () => {
            remainingAsyncOps--;
            if (remainingAsyncOps <= 0) {
                resolve();
            }
        });
    });
...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    return {
        ...
        getLineInfo(input, offset) {
            return getLineInfo(input, offset);
        },
...
class Queue {
    ...
    run(task, shouldRunImmediately = false) {
        return new Promise((resolve, reject) => {
            this.queue.push({ reject, resolve, task });
            this.work(shouldRunImmediately);
        });
    }
    async work(shouldRunImmediately) {
        if (this.workerCount >= this.maxParallel && !shouldRunImmediately)
            return;
        this.workerCount++;
        let entry;
...
class Graph {
    ...
    async build() {
        ...
        this.options.generateModuleGraphOnly || this.sortModules();
        timeEnd('sort and bind modules', 2);
        timeStart('mark included statements', 2);
        this.options.generateModuleGraphOnly || this.includeStatements();
...
async function normalizeInputOptions(config) {
    ...
    const options = {
        ...
        generateModuleGraphOnly: config.generateModuleGraphOnly ?? false,
        ...
        skipBuildEnd: config.skipBuildEnd || false,
...
async function normalizeOutputOptions(config, inputOptions, unsetInputOptions) {
    ...
    const outputOptions = {
        ...
        bundleMode: config.bundleMode ?? false,
...
async function rollupInternal(rawInputOptions, watcher) {
    ...
    await catchUnfinishedHookActions(graph.pluginDriver, async () => {
        try {
            ...
            await graph.pluginDriver.hookSeq('beforeBuild', []);
            await graph.pluginDriver.hookParallel('buildStart', [inputOptions]);
            await graph.pluginDriver.hookSeq('afterBuildStart', []);
            timeEnd('initialize', 2);
            await graph.build();
            timeStart('before build end', 2);
            await graph.pluginDriver.hookSeq('beforeBuildEnd', []);
            timeEnd('before build end', 2);
            if (inputOptions.skipBuildEnd) {
                return;
            }
            timeStart('build end', 2);
            await graph.pluginDriver.hookParallel('buildEnd', []);
            timeEnd('build end', 2);
            timeStart('after build end', 2);
            await graph.pluginDriver.hookParallel('afterBuildEnd', []);
            timeEnd('after build end', 2);
        }
        catch (error_) {
            ...
            if (watcher === null) {
                await graph.pluginDriver.hookParallel('cleanUp', []);
            }
            throw error_;
        }
    });
    ...
    const result = {
        ...
        async triggerBundleEnd(isWatch) {
            await graph.pluginDriver.hookParallel('bundleEnd', []);
            if (isWatch) {
                return;
            }
            await graph.pluginDriver.hookParallel('cleanUp', []);
...
function applyOptionHook(watchMode) {
    return async (inputOptions, plugin) => {
        ...
        return ((await handler.call({ meta: { rollupVersion: version$1, watchMode }, share: dataShare }, await inputOptions)) || inputOptions);
    };
}
...
async function mergeInputOptions(config, overrides, defaultOnWarnHandler) {
    ...
    const inputOptions = {
        ...
        endEvent: getOption('endEvent'),
        experimentalCacheExpiry: getOption('experimentalCacheExpiry'),
        external: getExternal(config, overrides),
        generateModuleGraphOnly: getOption('generateModuleGraphOnly'),
        ...
        skipBuildEnd: getOption('skipBuildEnd'),
        startEvent: getOption('startEvent'),
...
async function mergeOutputOptions(config, overrides, warn) {
    ...
    const outputOptions = {
        ...
        bundleMode: getOption('bundleMode'),
...
export { clearLastReUsedFiles, createFilter, defineConfig, fseventsImporter, getAugmentedNamespace, getLastReUsedFiles, picomatchExports, rollup, rollupInternal, version$1 as version, watch };
